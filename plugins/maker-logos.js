...
│⭔ ${usedPrefix}logo `
    ...
    const map2 = effects.map(v => v.title).join(
        `\n│⭔ ${usedPrefix}logo `
    )
...
🛡️⃟🏴‍☠ ════ •⊰❂⊱• ════ 🛡️⃟🏴‍☠️`
    ...
    if (!effect) {
        throw info2 + map2 + info3
    }

    const effectoSelect = effects.find(
        v => v.title.toLowerCase() === effect.toLowerCase()
    )

    if (!effectoSelect) {
        throw `❌ Effetto non trovato.\n\nUsa ${usedPrefix}${command} per vedere la lista degli effetti.`
    }

    let text = txt
        .replace(new RegExp(`^${escapeRegex(effect)}`, 'i'), '')
        .trim()

    if (!text) {
        throw `❌ Inserisci il testo.\n\nEsempio:\n${usedPrefix}${command} ${effect} Enemies`
    }
    ...
    if (!res?.image) {
        throw '❌ Errore durante la generazione del logo. Riprova tra qualche secondo.'
    }
...
function getInputValue(html, id) {
    ...
    const reverseRegex = new RegExp(
        `<input[^>]+value=["']([^"']*)["'][^>]+(?:id=["']${id}["']|name=["']${id}["'])[^>]*>`,
        'i'
    )

    return reverseRegex.exec(html)?.[1] || ''
}

function getRadioValues(html) {
    const values = []
    const regex = /<input\b[^>]*name=["']radio0\[radio\]["'][^>]*>/gi
...
function getFormValue(html) {
    let match = html.match(
        /<textarea[^>]+id=["']form_value["'][^>]*>([\s\S]*?)<\/textarea>/i
    )

    if (match?.[1]) return match[1].trim()

    match = html.match(
        /<textarea[^>]+id=["']form_value_input["'][^>]*>([\s\S]*?)<\/textarea>/i
    )

    if (match?.[1]) return match[1].trim()

    match = html.match(
        /<input[^>]+id=["']form_value["'][^>]+value=["']([^"']*)["']/i
    )

    if (match?.[1]) return match[1]

    match = html.match(
        /<input[^>]+id=["']form_value_input["'][^>]+value=["']([^"']*)["']/i
    )
...
async function maker(url, text) {
    if (!url || !/^https?:\/\/(?:www\.)?textpro\.me\//i.test(url)) {
        throw new Error('URL TextPro non valido')
    }
    ...
    try {
        ...
        const server = getInputValue(html, 'build_server')
        const serverId = getInputValue(html, 'build_server_id')
        const token = getInputValue(html, 'token')
        const submit = getInputValue(html, 'submit')

        if (!server || !serverId || !token) {
            throw new Error('Parametri TextPro non trovati')
        }

        const types = getRadioValues(html)
        ...
        const formValue = getFormValue(response.data)

        if (!formValue) {
            throw new Error('form_value non trovato')
        }
        ...
        try {
            ...
            throw new Error('form_value non contiene JSON valido')
        }

        const imageResponse = await axios.post(
            `${origin}/effect/create-image`,
            data,
            {
                headers: {
                    Accept: '*/*',
        ...
        if (!result?.success) {
            throw new Error('TextPro ha rifiutato la richiesta')
        }

        const image =
            result.fullsize_image ||
            result.image

        if (!image) {
            throw new Error('Immagine non restituita')
        }
